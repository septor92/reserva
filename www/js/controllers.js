...
.controller('DashCtrl', function($scope,$ionicPopup, $timeout,$cordovaSQLite) {
    ...
    $scope.guardar = function(persona){
        ...
        $cordovaSQLite.execute(db, 'INSERT INTO reserva (origen,destino,fechaini,fecharegre,personas,costo) VALUES (?,?,?,?,?,?)', [persona.origen,persona.destino,persona.fechaini,persona.fecharegre,persona.personas,persona.costo])
...
.controller('ChatDetailCtrl', function($scope, $stateParams, Chats,$cordovaSQLite) {
  ...
  $scope.guardar = function(persona){
        ...
        $cordovaSQLite.execute(db, 'UPDATE reserva set origen=?,destino=?,fechaini=?,fecharegre=?,personas=?,costo=? where id = ?', [persona.origen,persona.destino,persona.fechaini,persona.fecharegre,persona.personas,persona.costo,persona.id])
        .then(function(result) {
            $scope.statusMessage = "Registro guardado!";
        }, function(error) {
            $scope.statusMessage = "Error al guardar: " + error.message;
